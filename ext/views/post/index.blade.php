...
@foreach ($posts as $post)
    <div class="sm:grid grid-cols-2 gap-20  py-10  border-b border-gray-200">
        <div class="content-center">
            <img class="h-64 w-full" src="{{ $post->getFileLink('image')}}"  alt="">
        </div>
        <div>
            ...
            <p class="text-xl text-gray-700 pt-8 pb-10 leading-8 font-light">
                {{ strlen($post->body) > 50 ? substr($post->body,1,90).'...' : $post->body }}
            </p>

            <a href="/post/{{ $post->id }}" class="uppercase bg-blue-500 text-gray-100 font-bold text-sm py-3 px-3 rounded-3xl">
                Keep Reading
            </a>
